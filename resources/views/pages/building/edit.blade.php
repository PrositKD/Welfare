                ...
                <div class="mb-2">
                    ...
                    <select class="form-select basic-single" name="road_id" required>
                        ...
                        @foreach ($roads as $road)
                            <option value="{{ $road->id }}" {{ $building->road_id == $road->id ? 'selected' : '' }}>{{ $road->road_no }}{{ $road->block ? '/' . $road->block : '' }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-2">
                    <label for="contact_person" class="form-label">Contact Person Name<span class="text-danger">*</span></label>
                    <input type="text" name="contact_person" id="contact_person" class="form-control" placeholder="Contact Person Name" required value="{{$building->contact_person}}">
                </div>
                <div class="mb-2">
